...
./svauto.sh --cpu-check --hostname-check \
	...
	--ansible-playbook-builder="localhost,bootstrap;base_os_upgrade=yes,grub-conf,ubuntu-network-setup;ubuntu_primary_interface_via_ifupdown=yes;ubuntu_setup_dummy_nics=yes;ubuntu_enable_ip_masquerade=yes;os_neutron_lbr_enabled=yes,os_manage_etc_hosts,os_clients,ssh_keypair,os_mysql,os_rabbitmq,os_memcached,apache2,os_keystone,os_glance,ubuntu-virt;setup=host,os_nova;os_aio=yes;os_nova_ctrl=yes;os_nova_cmpt=yes,os_neutron;os_aio=yes;os_neutron_ctrl=yes;os_neutron_net=yes;os_neutron_lbr_enabled=yes;neutron_interface_driver=linuxbridge;os_external=$OS_EXTERNAL;os_data=$OS_DATA,os_horizon,os_heat,os_keypair,os_ext_net,download-images;download_group=cloud;download_ubuntu=yes,os_glance_images,post-cleanup"
